...
// NOTE: This implementation is basically untouched from CSimpleSprite
///////////////////////////////////////////////////////////////////////////////
//-----------------------------------------------------------------------------
#include <stdio.h>
#include <assert.h>
...
std::map<std::string, MySimpleSprite::sTextureDef > MySimpleSprite::m_textures;

//-----------------------------------------------------------------------------
MySimpleSprite::MySimpleSprite(const char* fileName, const unsigned int nColumns, const unsigned int nRows)
	: m_nColumns(nColumns)
	, m_nRows(nRows)
	, m_frame(0)
{
	// Prevent temp crash thanks to stupid static init content
...
void MySimpleSprite::Update(const float dt)
{
	if (m_currentAnim >= 0)
	{
...
void MySimpleSprite::CalculateUVs()
{
...
void MySimpleSprite::SetFrame(const unsigned int f)
{
	m_frame = f;
...
void MySimpleSprite::SetAnimation(const int id)
{
	SetAnimation(id, false);
}

void MySimpleSprite::SetAnimation(const int id, const bool playFromBeginning)
{
...
bool MySimpleSprite::LoadTexture(const std::string& filename)
{
